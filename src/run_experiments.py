"""
Main experiment runner for Active Feature-Value Acquisition (AFA) project.
v2: full candidate scoring (sample_fraction=1.0), N_TRIALS=20, AUC metric.

Compares:
  - Uniform baseline
  - SEU-US  (Sampled Expected Utility, uniform candidate sampling)
  - SEU-ES  (Sampled Expected Utility, error/uncertainty sampling)
  - SEU-Accuracy (ablation: accuracy instead of log-gain)

Under two missing-rate conditions: 10% and 20%.
Results saved with prefix "results_v2_*".
...
BATCH_SIZE      = 10
RESULT_PREFIX   = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, "results", "results_v2")
...
def run_all(X_full, y, missing_rate, n_trials=N_TRIALS):
    """
    Run all strategies for a given missing_rate over n_trials seeds.
    ...
    for strategy in STRATEGIES:
        ...
        for seed in range(n_trials):
            ...
            # AUC: area under accuracy-vs-cost curve (normalised by total cost)
            auc = np.trapz(interp, grid) / grid[-1]
...
def plot_early_stage(results_by_rate, ceiling_acc, prefix=RESULT_PREFIX):
    fig, axes = plt.subplots(1, 2, figsize=(13, 4.5), sharey=True)
    for ax, mr in zip(axes, MISSING_RATES):
        results = results_by_rate[mr]
...
def print_summary_table(results_by_rate, ceiling_acc):
    checkpoints = [0.25, 0.50, 0.75, 1.0]
    for mr in MISSING_RATES:
        print(f"\n{'='*72}")
        print(f"Missing rate: {int(mr*100)}%")
        total_cost = int(X_full.shape[0] * X_full.shape[1] * mr)
        header = f"{'Strategy':<16}" + "".join(f"  @{int(c*100)}%cost" for c in checkpoints)
        header += "        AUC"
        print(header)
        print("-" * 72)
        for strategy in STRATEGIES:
            grid, mean_acc, std_acc, mean_auc, std_auc = results_by_rate[mr][strategy]
            row = f"{STRATEGY_LABELS[strategy]:<16}"
            ...
            print(row)
        print("-" * 72)
        print(f"{'Fully Observed':<16}  {ceiling_acc:.4f}  (upper bound)")
    print(f"\n{'='*72}")
...
if __name__ == "__main__":
    ...
    print(f"Fully observed upper bound: {ceiling_acc:.4f}")

    # Run verbose debug for SEU log-gain (1 trial)
    print("\nRunning verbose utility debug for SEU-US (log-gain) ...")
    debug_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, "docs", "utility_debug.txt")
    if os.path.exists(debug_path):
        os.remove(debug_path)
    run_acquisition(X_full, y, missing_rate=0.20, strategy="seu-us",
                    seed=0, sample_fraction=1.0,
                    verbose=True, debug_file=debug_path)
    print(f"  Utility debug saved: {debug_path}")
    ...
    plot_early_stage(results_by_rate, ceiling_acc)

    print_summary_table(results_by_rate, ceiling_acc)
    print("\nDone.")
